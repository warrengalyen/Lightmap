...
export class GrabModeDragOperation extends BaseDragOperation {
    ...
    cancel(): void {
        ...
        if (this.doorId && this.originalDoorPosition !== null) {
            this.callbacks.onUpdateDoorPosition(
                this.doorId,
                this.originalDoorPosition,
            );
        }
    ...
    private updateDoor(mousePos: Vector2): void {
        ...
        const { normalized: normalizedDir, length: wallLength } =
        ...
        let newPosition =
            mouseToWallStart.x * normalizedDir.x +
            mouseToWallStart.y * normalizedDir.y;
        ...
        const otherDoors = this.config
            .getDoorsByWallId(door.wallId)
            .filter((d) => d.id !== this.doorId);
        newPosition = this.avoidDoorOverlap(
            newPosition,
            door.width,
            otherDoors,
            minPosition,
    ...
    private handleShiftSnapping(targetPos: Vector2): {
        ...
        if (
            this.selection.selectedVertexIndices.size === 1 &&
        ...
        if (
            this.selection.selectedLightIds.size === 1 &&
