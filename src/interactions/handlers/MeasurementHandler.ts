...
export class MeasurementHandler extends BaseInteractionHandler {
    ...
    constructor(
        config: MeasurementHandlerConfig,
        callbacks: MeasurementHandlerCallbacks,
    ) {
        super();
        this.config = config;
    ...
    private handleClickFromLight(
        ...
        const clickedLight = lightManager.getLightAt(
            event.worldPos,
            LIGHT_HIT_TOLERANCE_FT,
        );
        ...
        if (
            clickedLight &&
            clickedLight.id === measurementController.sourceLightId
        ) {
        ...
        if (
            ...
            measurementController.setTargetLight(
                clickedLight.id,
                clickedLight.position,
            );
            this.updateDisplay();
            return true;
        }
        ...
        if (clickedVertexIndex !== null) {
            measurementController.setTargetVertex(
                clickedVertexIndex,
                vertices[clickedVertexIndex],
            );
            this.updateDisplay();
            return true;
        }
