...
export interface BoxSelectionHandlerCallbacks {
    ...
    onBoxSelectionComplete: (vertexIndices: number[], lightIds: string[], addToSelection: boolean) => void;
...
export class BoxSelectionHandler extends BaseInteractionHandler {
    ...
    canHandle(_event: InputEvent, context: InteractionContext): boolean {
        ...
        return (
            state.isSelecting || (context.roomState.isClosed && !context.isDrawingEnabled && !context.isPlacingLights)
        );
    }
