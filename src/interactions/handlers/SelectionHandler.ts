...
export class SelectionHandler extends BaseInteractionHandler {
    ...
    private trySelectVertex(
        ...
        const vertexIndex = findVertexAtPosition(
            pos,
            vertices,
            VERTEX_HIT_TOLERANCE_FT,
        );
        ...
        // Click on already-selected vertex with multiple items: start multi-drag
            ...
            isAlreadySelected &&
            (selectedIndices.size > 1 || selectedLightIds.size > 0)
        ) {
            this.startUnifiedDrag(vertexIndex, null, pos, vertices);
        }
    ...
    private trySelectLight(
        ...
        const light = this.config.lightManager.getLightAt(
            pos,
            LIGHT_HIT_TOLERANCE_FT,
        );
        ...
        // Click on already-selected light with multiple items: start multi-drag
            ...
            isAlreadySelected &&
            (selectedLightIds.size > 1 || selectedIndices.size > 0)
        ) {
            this.startUnifiedDrag(null, light.id, pos, vertices);
        }
    ...
    private getSelectionOrigin(
        ...
        if (selection.selectedLightIds.size > 0) {
            ...
            const light = context.roomState.lights.find(
                (l) => l.id === firstId,
            );
        ...
        if (selection.selectedWallId) {
            const wall = context.roomState.walls.find(
                (w) => w.id === selection.selectedWallId,
            );
            if (wall) {
                return {
        ...
        if (selection.selectedDoorId) {
            const door = context.roomState.doors.find(
                (d) => d.id === selection.selectedDoorId,
            );
            if (door) {
                const wall = context.roomState.walls.find(
                    (w) => w.id === door.wallId,
