...
const metricsInputs = derived([roomStore, roomBounds, lightingStatsConfig], ([$room, $bounds, $config]) => ({
    lights: $room.lights,
    walls: $room.walls,
...
export const lightingMetrics = derived<typeof debouncedInputs, LightingMetrics | null>(debouncedInputs, ($inputs) => {
    if (!$inputs.visible) {
        return null;
    }
