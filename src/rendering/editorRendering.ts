 ...
 */
export function getWallSegmentsWithDoors(
    wall: WallSegment,
    doors: Door[],
): WallSegmentPart[] {
...
export function createWallLine(
    ...
    const material = new THREE.LineBasicMaterial({
        ...
        linewidth: isSelected
            ? theme.editor.wallLineWidthSelected
            : theme.editor.wallLineWidth,
    });
...
export function createDrawingVertex(
    pos: Vector2,
    isStart: boolean,
): THREE.Mesh {
    const theme = getTheme();
    ...
    const material = new THREE.MeshBasicMaterial({
        color: isStart
            ? theme.editor.drawingVertexStart
            : theme.editor.drawingVertex,
    });
    const mesh = new THREE.Mesh(geometry, material);
...
export function createDrawingLine(start: Vector2, end: Vector2): THREE.Line {
    ...
    const geometry = new THREE.BufferGeometry().setFromPoints(points);
    const material = new THREE.LineBasicMaterial({
        color: theme.editor.drawingLine,
    });
...
export function createDoorGraphics(
    ...
    const doorColor = isSelected
        ? theme.editor.doorSelected
        : theme.editor.door;
    const arcColor = isSelected
        ? theme.editor.doorSelected
        : theme.editor.doorArc;
